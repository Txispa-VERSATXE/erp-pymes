...
<head>
    ...
    <style>
        ...
        .badge-recibido  { background: #dcfce7; color: #166534; padding: 2px 6px; border-radius: 4px; }
...
<body>
    <div class="header">
        <h1>ERP PYMES — Listado de Compras</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>#{{ strtoupper(substr($compra->id, -6)) }}</td>
                <td>{{ $proveedor->nombre ?? '—' }}</td>
                <td>{{ $compra->fecha_compra }}</td>
                <td>{{ number_format($compra->total, 2, ',', '.') }} €</td>
                <td><span class="badge-{{ $compra->estado }}">{{ ucfirst($compra->estado) }}</span></td>
            </tr>
            @endforeach
            <tr class="total-row">
                <td colspan="3">Total</td>
                <td>{{ number_format($compras->sum('total'), 2, ',', '.') }} €</td>
                <td>{{ $compras->count() }} compras</td>
            </tr>
        </tbody>
    </table>
